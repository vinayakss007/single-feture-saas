...
Wedding parameters in, 12-category budget with city/season multipliers and savings plan out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — Personal finance
- **Built for** — Indian couples and families planning weddings, wedding planners building budgets
...
```bash
curl https://weddingbudget.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://weddingbudget.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $WEDDINGBUDGET_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "weddingbudget": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://weddingbudget.abetworks.in",
...
Exposes one tool, `weddingbudget_calculate`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/34-weddingbudget`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t weddingbudget .
docker run -p 3000:3000 -e API_KEYS=your_key weddingbudget
```
