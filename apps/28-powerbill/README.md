...
```bash
curl https://powerbill.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://powerbill.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $POWERBILL_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "powerbill": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://powerbill.abetworks.in",
...
**Vercel** — set the root directory to `apps/28-powerbill`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t powerbill .
docker run -p 3000:3000 -e API_KEYS=your_key powerbill
```
