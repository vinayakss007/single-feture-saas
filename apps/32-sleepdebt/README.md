...
Sleep log in, cumulative debt, severity, recovery plan, and tonight's bedtime out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — Health & wellness
- **Built for** — Professionals with irregular sleep, shift workers, students, anyone tracking sleep debt
...
```bash
curl https://sleepdebt.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://sleepdebt.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $SLEEPDEBT_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "sleepdebt": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://sleepdebt.abetworks.in",
...
Exposes one tool, `sleepdebt_analyze_log`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/32-sleepdebt`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t sleepdebt .
docker run -p 3000:3000 -e API_KEYS=your_key sleepdebt
```
