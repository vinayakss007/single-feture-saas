...
Annual CTC in, full payslip with PF, ESI, PT and TDS out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — Payroll compliance
- **Built for** — Small employers, HR consultants, chartered accountants and anyone doing payroll in a spreadsheet
...
```bash
curl https://payslipin.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://payslipin.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $PAYSLIPIN_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "payslipin": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://payslipin.abetworks.in",
...
Exposes one tool, `payslipin_compute_payslip`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/18-payslipin`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t payslipin .
docker run -p 3000:3000 -e API_KEYS=your_key payslipin
```
