...
Sanction letter terms in, true APR and amortisation out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — Personal finance
- **Built for** — Anyone taking a home, car, personal or business loan, and anyone already paying one off
...
```bash
curl https://loantruth.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://loantruth.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $LOANTRUTH_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "loantruth": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://loantruth.abetworks.in",
...
Exposes one tool, `loantruth_analyse_loan`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/24-loantruth`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t loantruth .
docker run -p 3000:3000 -e API_KEYS=your_key loantruth
```
