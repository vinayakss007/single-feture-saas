...
Two pricing page snapshots in, classified commercial diff out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — Competitive intelligence
- **Built for** — Product marketing, pricing owners, competitive intelligence, founders
...
```bash
curl https://pricepulse.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://pricepulse.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $PRICEPULSE_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "pricepulse": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://pricepulse.abetworks.in",
...
Exposes one tool, `pricepulse_diff_pricing`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/03-pricepulse`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t pricepulse .
docker run -p 3000:3000 -e API_KEYS=your_key pricepulse
```
