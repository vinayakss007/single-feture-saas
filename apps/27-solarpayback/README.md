...
Monthly bill and rooftop area in, payback period and savings out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — Home energy
- **Built for** — Homeowners considering rooftop solar, and anyone who was just quoted a system
...
```bash
curl https://solarpayback.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://solarpayback.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $SOLARPAYBACK_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "solarpayback": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://solarpayback.abetworks.in",
...
Exposes one tool, `solarpayback_analyse_roi`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/27-solarpayback`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t solarpayback .
docker run -p 3000:3000 -e API_KEYS=your_key solarpayback
```
