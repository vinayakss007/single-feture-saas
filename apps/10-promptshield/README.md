...
Untrusted text in, verdict plus redacted text out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — AI security
- **Built for** — Teams shipping AI agents to production, platform and security engineers
...
```bash
curl https://promptshield.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://promptshield.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $PROMPTSHIELD_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "promptshield": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://promptshield.abetworks.in",
...
Exposes one tool, `promptshield_scan_text`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/10-promptshield`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t promptshield .
docker run -p 3000:3000 -e API_KEYS=your_key promptshield
```
