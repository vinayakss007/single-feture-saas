...
Resume text and job description in, ATS parse score with keyword match rate and rejection flags out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — Career tools
- **Built for** — Job seekers in India and globally, career coaches, resume writers, placement consultants
...
```bash
curl https://resumeats.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://resumeats.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $RESUMEATS_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "resumeats": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://resumeats.abetworks.in",
...
Exposes one tool, `resume_ats_scan`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/36-resumeats`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t resumeats .
docker run -p 3000:3000 -e API_KEYS=your_key resumeats
```
