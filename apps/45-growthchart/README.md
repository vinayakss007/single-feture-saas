...
Child date of birth, sex, and measurements over time in, exact WHO percentiles via LMS method, growth velocity, percentile crossing alerts, and paediatrician discussion prompts out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — Health tools
- **Built for** — Parents, paediatricians, anganwadi workers, child health clinics, school health programmes
...
```bash
curl https://growthchart.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://growthchart.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $GROWTHCHART_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "growthchart": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://growthchart.abetworks.in",
...
Exposes one tool, `child_growth_chart`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/45-growthchart`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t growthchart .
docker run -p 3000:3000 -e API_KEYS=your_key growthchart
```
