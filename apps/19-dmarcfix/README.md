...
Email auth records in, lookup count, failures and corrected records out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — Email deliverability
- **Built for** — Founders whose email lands in spam, ops and platform engineers, agencies running client domains
...
```bash
curl https://dmarcfix.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://dmarcfix.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $DMARCFIX_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "dmarcfix": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://dmarcfix.abetworks.in",
...
Exposes one tool, `dmarcfix_audit_records`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/19-dmarcfix`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t dmarcfix .
docker run -p 3000:3000 -e API_KEYS=your_key dmarcfix
```
