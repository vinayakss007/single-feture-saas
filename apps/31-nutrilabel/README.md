...
Nutrition facts in, traffic-light breakdown and label honesty rating out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — Health & nutrition
- **Built for** — Health-conscious Indian consumers, dietitians, fitness coaches, parents reading labels for their kids
...
```bash
curl https://nutrilabel.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://nutrilabel.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $NUTRILABEL_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "nutrilabel": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://nutrilabel.abetworks.in",
...
Exposes one tool, `nutrilabel_analyze_label`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/31-nutrilabel`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t nutrilabel .
docker run -p 3000:3000 -e API_KEYS=your_key nutrilabel
```
