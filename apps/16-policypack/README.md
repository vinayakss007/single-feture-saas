...
Company profile in, policy set with control mapping and gap list out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — Security compliance
- **Built for** — Startups facing their first security review, CTOs, and whoever got handed the questionnaire
...
```bash
curl https://policypack.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://policypack.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $POLICYPACK_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "policypack": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://policypack.abetworks.in",
...
Exposes one tool, `policypack_generate_policies`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/16-policypack`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t policypack .
docker run -p 3000:3000 -e API_KEYS=your_key policypack
```
