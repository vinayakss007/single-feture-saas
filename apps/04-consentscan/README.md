...
URL in, prioritised DPDP and GDPR finding list out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — Privacy compliance
- **Built for** — Indian SMBs, web agencies, anyone with EU users, compliance owners
...
```bash
curl https://consentscan.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://consentscan.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $CONSENTSCAN_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "consentscan": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://consentscan.abetworks.in",
...
Exposes one tool, `consentscan_audit_site`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/04-consentscan`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t consentscan .
docker run -p 3000:3000 -e API_KEYS=your_key consentscan
```
