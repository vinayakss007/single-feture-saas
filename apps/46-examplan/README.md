...
Exam date, subjects with syllabus size and difficulty, and study hours per day in, weighted hour allocation, day-by-day calendar, revision blocks, insufficiency warnings, and critical subjects out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — Education tools
- **Built for** — Students (board exams, competitive exams, university), parents, tutors, coaching centres
...
```bash
curl https://examplan.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://examplan.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $EXAMPLAN_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "examplan": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://examplan.abetworks.in",
...
Exposes one tool, `exam_study_planner`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/46-examplan`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t examplan .
docker run -p 3000:3000 -e API_KEYS=your_key examplan
```
