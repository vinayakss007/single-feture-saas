...
Race distance and training data in, per-km pace plan with three strategies out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — Fitness & endurance
- **Built for** — Recreational runners preparing for 5K to marathon, running coaches building race strategies
...
```bash
curl https://racepace.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://racepace.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $RACEPACE_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "racepace": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://racepace.abetworks.in",
...
Exposes one tool, `racepace_plan_race`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/33-racepace`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t racepace .
docker run -p 3000:3000 -e API_KEYS=your_key racepace
```
