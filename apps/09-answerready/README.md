...
URL in, AI answer-engine readiness score and the two missing files out. One job, four surfaces: a marketing site, a working app, a REST endpoint and an MCP server.

- **Category** — AI search optimisation
- **Built for** — SEO owners, content marketers, founders losing traffic to AI answers
...
```bash
curl https://answerready.abetworks.in/api/v1/run
```

That returns the input schema and a complete working example. To run it:

```bash
curl -X POST https://answerready.abetworks.in/api/v1/run \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $ANSWERREADY_KEY" \
  -d @payload.json
```
...
```json
{
  "mcpServers": {
    "answerready": {
      ...
      "args": ["./mcp/server.mjs"],
      "env": {
        "SFS_API_URL": "https://answerready.abetworks.in",
...
Exposes one tool, `answerready_audit_page`. The tool schema is fetched from `GET /api/v1/run` at startup, so the agent-facing contract can never drift from the REST contract.

## Deploy

**Vercel** — set the root directory to `apps/09-answerready`, framework Next.js. Nothing else to configure.

**Docker**

```bash
docker build -t answerready .
docker run -p 3000:3000 -e API_KEYS=your_key answerready
```
